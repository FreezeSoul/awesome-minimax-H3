...
| Handheld Shaky Cam | ![Camera][ltype-camera] | Handheld / shaky-cam camera movement; Civitai mirror with no trigger word or training details documented. Pairs with the other camera LoRAs — use one per shot. | 74 MB | [![][gh-neph1]](https://huggingface.co/neph1/minimax_h3_handheld_shaky_camera) |
...
| Character Swap v1 | ![Utility][ltype-utility] | Whole-character replacement for Ref2VA: the source clip is `<Video 1>`, the replacement is `<Picture 1>`, and the prompt names who to swap while preserving the rest of the scene. No trigger word (trained on instruction-style captions); strength 1.0. Trained by Akatz Labs for 1,000 updates on the pruned INT8 ConvRot base with the frozen [ostris Ref2VA training assistant](https://huggingface.co/ostris/minimax_h3_training_adapter) (not merged, not redistributed); dataset + training config, launcher and SHA256SUMS ship in the repo. Experimental — motion timing, expressions and hard cuts stay unreliable, and short continuous 4–5 s shots worked far better than 14 s ones. | 148 MB | [![][gh-akatz--ai]](https://huggingface.co/akatz-ai/MiniMax-H3-Character-Swap-LoRA) |
