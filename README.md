...
* [Playtime-AI/Minimax_H3-Ricky_Gervais](https://huggingface.co/Playtime-AI/Minimax_H3-Ricky_Gervais) - Character LoRA of comedian Ricky Gervais (`MM-H3 - Ricky Gervais.safetensors`, 148 MB). Same Playtime-AI series as above; sample clip included, minimal card (Apache-2.0).
...
* JOKER141
  ...
  * [MiniMax-H3-Weapon-Combat-LoRA](https://huggingface.co/JOKER141/MiniMax-H3-Weapon-Combat-LoRA) - **Weapon combat** — specialized weapon-combat motion LoRA: weapon-trajectory continuity (large swings, rotational attacks, recoveries, weapon-state inheritance), attack/defense interaction (blocking, parrying, deflection, sliding contact, weapon separation), and spatial combat movement (pursuit, retreat, lateral movement, circling). Trigger `BUNNY 🐇`; works mounted or on foot, in group combat. Ships demo clips + one-click RunningHub workflows. (`Bunny_weapon_combatV1.safetensors`, 148 MB)
...
* [shamanic/minimax-h3-equi360-lora](https://huggingface.co/shamanic/minimax-h3-equi360-lora) - **Equirectangular 360° VR** — full-sphere monoscopic equirectangular 360° video with native audio; trigger `equirect360`, then describe environment / motion / sound. Current default is the reviewed-v2 checkpoint (57 reviewed clips, 36 sources, 16 scene families — stricter watermark/disconnected-content filtering than the cave-heavy v1, which remains under `versions/v1/`); 4-prompt A/B vs a 119-clip candidate marginally favored it. Trained via fal; full training/captioning scripts + trainer configs in the repo. MiniMax Community License. (125 MB)
...
* Jojocodex
  ...
  * [wushu-action-v7-minimax-h3-fl2va-ref2va-lora](https://huggingface.co/Jojocodex/wushu-action-v7-minimax-h3-fl2va-ref2va-lora) - **Wushu / martial-arts action v7 (E3)** — the current "E3" build supersedes all earlier pruned-base files (removed by the author): trained on the **unpruned full INT8-ConvRot DiT + bf16 Qwen3-VL TE** (924 curated fight clips, 832×480 @ 24 fps, three-stage professional technique captions), 1000/2000-step checkpoints. Trigger `wushu_action`; pair with JOKER141's Combat-Base-V2 / Motion-Continuity-Repair for best results, or a little of the Spatial & Physics LoRA for physical compensation. Sampler: euler/simple, CFG 1.0, ~25 steps on int8 base; known issue: low step counts render soft — raise steps. Ships the multi-reference two-pass fight workflow JSON + 招式TAGS完整清单.md tag list + a duel-sim HTML. (`wushu_action_v7_fl2va_aitoolkit_adaln_full-int8convrot_bf16te_1000step.safetensors`, 569 MB · 2000-step variant same size)
...
| [ComfyUI-Viggle-Animate-H3](https://github.com/Saganaki22/ComfyUI-Viggle-Animate-H3) | Saganaki22 | ![Conditioning][cat-cond] | ComfyUI nodes for the [Viggle-Animate](https://huggingface.co/Viggle/Viggle-Animate) 33.1 B ref2va finetune (character replacement: driving video + reference still → identity from the image, motion/camera/timing from the clip; no text encoder — one frozen 362-token Qwen3-VL embedding for every render). DMD2-distilled sampler at 4–8 steps (6 = sweet spot; manual sigma schedules derived from the upstream shift-3 formula reproduce the distilled schedule via stock `ManualSigmas`). v1.3.0 adds windowed conditioning + the **Viggle Chunked Sampler** for longer clips (latent carry, chunk reuse, seed overrides), plus chunk-loop nodes with disk checkpoints and external VAE decode for long runs. Apache-2.0. |
...
| [ComfyUI-Minimax-H3-Extender (pmhaidn)](https://github.com/pmhaidn/ComfyUI-Minimax-H3-Extender) | pmhaidn | ![Conditioning][cat-cond] | Video + phase-locked audio extension pack — carry motion and temporal context from a previous clip into the next with frame-accurate continuity. `MiniMaxH3VideoExtender` (pure-latent or decoded-video input), Farneback optical-flow anti-stutter seam smoothing, seam-fade flicker-free color alignment, smart passthrough for single-clip workflows, `MiniMaxH3VideoMerge` lossless concat (auto resolution/audio-rate align), `MiniMaxH3LoopTrim` overlap trimming, and a Turbo-LoRA hot-patch that intercepts AdaLN tensor ops in ComfyUI-MiniMax-H3-Turbo to prevent shape-mismatch crashes with multimodal references. Ships latent-mode (recommended) and MP4-mode example workflows. GPL-3.0. |
...
| [Vision Prompt Composer](https://github.com/gabxav/ComfyUI-Vision-Prompt-Composer) | gabxav | ![Prompt][cat-prompt] | Composes one text from multiple image references — reuses the native `Generate Text` logic but replaces its single `image` input with eight optional inputs (`image_1`–`image_8`); `<Picture N>` numbering follows connected order, batches get per-item numbers, no collage/resize. Tested with Qwen3.8 27B via native `CLIPLoader` (w4a8 convrot); the bundled example workflow is configured for MiniMax H3 Ref2VA, but the node outputs plain text for any target model. |
...
| [H3-LongVideos](https://huggingface.co/Smite79/MiniMax-H3-Longvideos) | Smite79 | ![Conditioning][cat-cond] | Long H3 video with synchronized audio from a single written scene — turns a paragraph-structured script (scene + beats; one paragraph = one shot) into a chain of ~15 s shots joined into one continuous clip. The node injects continuity sentences on top of your verbatim text: character-sheet resolution (pronoun logic, per-shot reference tagging via `<Picture N>` → `ref_image_1…4` sockets), wardrobe tracking (`auto_remove` garment logic), restraint/state continuity (staged changes get both ends), gaze control (`hold_gaze`), and audio discipline (speech opens only on quoted lines / written sounds / effort beats; `mouths_shut_when_no_line` keeps mouths closed on wordless shots). `plan_only` previews the shot split + warnings without rendering. Requires ComfyUI 0.31+ with native H3 support; a hybrid fl2va/ref2va merge works best. Constant WIP — HF-hosted (not GitHub); Ko-fi-supported. |
...
| [INT8 I2V (javano2609.1.2)](https://huggingface.co/javawock7618/comfy-MiniMax-H3-workflows/resolve/main/MiniMax_int8_I2V-javano2609.1.2.json) | javawock7618 | I2VA | INT8 low-VRAM image-to-video workflow — FLF/I2V with Extend, ControlNet Union V2V, and Latent Upscaler. Older 2608.x/2609.1 files were removed from the repo; this is the current version. |
...
| [INT8 R2V (javano2609.1.2)](https://huggingface.co/javawock7618/comfy-MiniMax-H3-workflows/resolve/main/MiniMax_int8_R2V-javano2609.1.2.json) | javawock7618 | Ref2VA | INT8 low-VRAM reference-to-video workflow — Reference LoRA, ControlNet Union V2V, Extend, and Latent Upscaler. Current version (older 2608.x files removed from the repo). |
